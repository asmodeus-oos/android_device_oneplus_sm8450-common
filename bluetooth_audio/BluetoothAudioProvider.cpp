...
ndk::ScopedAStatus BluetoothAudioProvider::startSession(
    ...
    BluetoothAudioProviderContext* cookie = new BluetoothAudioProviderContext{session_type_};
...
ndk::ScopedAStatus BluetoothAudioProvider::endSession() {
    ...
    if (stack_iface_ != nullptr) {
        ...
        LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
    }
...
ndk::ScopedAStatus BluetoothAudioProvider::streamStarted(BluetoothAudioStatus status) {
    if (stack_iface_ != nullptr) {
        LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
                  << ", status=" << toString(status);
        BluetoothAudioSessionReport::ReportControlStatus(session_type_, true, status);
    } else {
...
ndk::ScopedAStatus BluetoothAudioProvider::streamSuspended(BluetoothAudioStatus status) {
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
              << ", status=" << toString(status);

    if (stack_iface_ != nullptr) {
        BluetoothAudioSessionReport::ReportControlStatus(session_type_, false, status);
    } else {
        LOG(WARNING) << __func__ << " - SessionType=" << toString(session_type_)
                     << ", status=" << toString(status) << " has NO session";
    }
...
ndk::ScopedAStatus BluetoothAudioProvider::updateAudioConfiguration(
    ...
    audio_config_ = std::make_unique<AudioConfiguration>(audio_config);
    BluetoothAudioSessionReport::ReportAudioConfigChanged(session_type_, *audio_config_);
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
...
ndk::ScopedAStatus BluetoothAudioProvider::setLowLatencyModeAllowed(bool allowed) {
    ...
    BluetoothAudioSessionReport::ReportLowLatencyModeAllowedChanged(session_type_, allowed);
    return ndk::ScopedAStatus::ok();
}
...
ndk::ScopedAStatus BluetoothAudioProvider::parseA2dpConfiguration(
    ...
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_) << " is illegal";
    return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
}
...
ndk::ScopedAStatus BluetoothAudioProvider::getLeAudioBroadcastDatapathConfiguration(
