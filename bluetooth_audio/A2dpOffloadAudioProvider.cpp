...
A2dpOffloadAudioProvider::A2dpOffloadAudioProvider(const A2dpOffloadCodecFactory& codec_factory)
    ...
    if (audio_config.getTag() == AudioConfiguration::Tag::a2dp) {
        ...
        if (!codec) {
            LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
                      << " - CodecId=" << a2dp_config.codecId.toString() << " is not found";
...
ndk::ScopedAStatus A2dpOffloadAudioProvider::onSessionReady(DataMQDesc* _aidl_return) {
    *_aidl_return = DataMQDesc();
...
ndk::ScopedAStatus A2dpOffloadAudioProvider::getA2dpConfiguration(
    ...
    *_aidl_return = std::nullopt;
    ...
    if (codec_factory_.GetConfiguration(remote_a2dp_capabilities, hint, &avdtp_configuration))
