...
LeAudioSoftwareBroadcastOutputAudioProvider::LeAudioSoftwareBroadcastOutputAudioProvider()
    : LeAudioSoftwareAudioProvider() {
    session_type_ = SessionType::LE_AUDIO_BROADCAST_SOFTWARE_ENCODING_DATAPATH;
}

LeAudioSoftwareBroadcastInputAudioProvider::LeAudioSoftwareBroadcastInputAudioProvider()
    : LeAudioSoftwareAudioProvider() {
...
ndk::ScopedAStatus LeAudioSoftwareAudioProvider::startSession(
    ...
    if (!BluetoothAudioCodecs::IsSoftwarePcmConfigurationValid(pcm_config)) {
        LOG(WARNING) << __func__ << " - Unsupported PCM Configuration=" << pcm_config.toString();
        return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
    }
    ...
    int bytes_per_sample = (pcm_config.bitsPerSample == 24) ? 4 : (pcm_config.bitsPerSample / 8);

    uint32_t data_mq_size = (ceil(pcm_config.sampleRateHz) / 1000) *
                            channel_mode_to_channel_count(pcm_config.channelMode) *
                            bytes_per_sample * (pcm_config.dataIntervalUs / 1000) * buffer_modifier;
    if (data_mq_size <= 0) {
        LOG(ERROR) << __func__ << "Unexpected audio buffer size: " << data_mq_size
                   ...
                   << ", BitsPerSample: " << static_cast<int>(pcm_config.bitsPerSample)
    ...
    LOG(INFO) << __func__ << " - size of audio buffer " << data_mq_size << " byte(s)";

    std::unique_ptr<DataMQ> temp_data_mq(new DataMQ(data_mq_size, /* EventFlag */ true));
...
ndk::ScopedAStatus LeAudioSoftwareAudioProvider::onSessionReady(DataMQDesc* _aidl_return) {
    ...
    *_aidl_return = data_mq_->dupeDesc();
    ...
    BluetoothAudioSessionReport::OnSessionStarted(session_type_, stack_iface_, &desc,
