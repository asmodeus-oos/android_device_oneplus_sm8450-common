...
HearingAidAudioProvider::HearingAidAudioProvider() : BluetoothAudioProvider(), data_mq_(nullptr) {
    LOG(INFO) << __func__ << " - size of audio buffer " << kDataMqSize << " byte(s)";
    std::unique_ptr<DataMQ> data_mq(new DataMQ(kDataMqSize, /* EventFlag */ true));
...
ndk::ScopedAStatus HearingAidAudioProvider::onSessionReady(DataMQDesc* _aidl_return) {
