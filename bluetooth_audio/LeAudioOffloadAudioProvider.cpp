...
std::map<CodecSpecificCapabilitiesLtv::Tag, CodecSpecificConfigurationLtv::Tag> cap_to_cfg_tag_map =
        {
...
std::map<int32_t, CodecSpecificConfigurationLtv::SamplingFrequency> sampling_freq_map = {
...
std::map<int32_t, CodecSpecificConfigurationLtv::FrameDuration> frame_duration_map = {
...
LeAudioOffloadPeripheralOutputAudioProvider::LeAudioOffloadPeripheralOutputAudioProvider()
    : LeAudioOffloadAudioProvider() {
    session_type_ = SessionType::LE_AUDIO_PERIPHERAL_OFFLOAD_ENCODING_DATAPATH;
}

LeAudioOffloadPeripheralInputAudioProvider::LeAudioOffloadPeripheralInputAudioProvider()
    : LeAudioOffloadAudioProvider() {
    session_type_ = SessionType::LE_AUDIO_PERIPHERAL_OFFLOAD_DECODING_DATAPATH;
}

LeAudioOffloadBroadcastOutputAudioProvider::LeAudioOffloadBroadcastOutputAudioProvider()
    : LeAudioOffloadAudioProvider() {
    session_type_ = SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH;
}

LeAudioOffloadBroadcastInputAudioProvider::LeAudioOffloadBroadcastInputAudioProvider()
    : LeAudioOffloadAudioProvider() {
    session_type_ = SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_DECODING_DATAPATH;
}
    ...
    if (session_type_ == SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH ||
        ...
        if (audio_config.getTag() != AudioConfiguration::leAudioBroadcastConfig) {
            LOG(WARNING) << __func__
                         << " - Invalid Audio Configuration=" << audio_config.toString();
...
ndk::ScopedAStatus LeAudioOffloadAudioProvider::onSessionReady(DataMQDesc* _aidl_return) {
    BluetoothAudioSessionReport::OnSessionStarted(session_type_, stack_iface_, nullptr,
                                                  *audio_config_, latency_modes_);
    *_aidl_return = DataMQDesc();
    return ndk::ScopedAStatus::ok();
}
ndk::ScopedAStatus LeAudioOffloadAudioProvider::setCodecPriority(const CodecId& in_codecId,
                                                                 int32_t in_priority) {
    codec_priority_map_[in_codecId] = in_priority;
    return ndk::ScopedAStatus::ok();
};

bool LeAudioOffloadAudioProvider::isMatchedValidCodec(CodecId cfg_codec, CodecId req_codec) {
    auto priority = codec_priority_map_.find(cfg_codec);
    if (priority != codec_priority_map_.end() &&
        priority->second == LeAudioOffloadAudioProvider::CODEC_PRIORITY_DISABLED) {
        return false;
    }
...
bool LeAudioOffloadAudioProvider::filterCapabilitiesMatchedContext(
    ...
    for (auto metadata : capabilities.metadata.value()) {
        ...
        if (metadata.value().getTag() == MetadataLtv::Tag::preferredAudioContexts) {
            ...
            auto& prefer_context =
                    metadata.value().get<MetadataLtv::Tag::preferredAudioContexts>().values;
...
bool LeAudioOffloadAudioProvider::isMatchedSamplingFreq(
        ...
        CodecSpecificCapabilitiesLtv::SupportedSamplingFrequencies& capability_freq) {
...
bool LeAudioOffloadAudioProvider::isMatchedFrameDuration(
        ...
        CodecSpecificCapabilitiesLtv::SupportedFrameDurations& capability_fduration) {
...
bool LeAudioOffloadAudioProvider::isMatchedAudioChannel(
        ...
        CodecSpecificCapabilitiesLtv::SupportedAudioChannelCounts& capability_channel) {
    int count = getCountFromBitmask(cfg_channel.bitmask);
    if (count == 1 && !(capability_channel.bitmask &
                        CodecSpecificCapabilitiesLtv::SupportedAudioChannelCounts::ONE))
        return false;
    if (count == 2 && !(capability_channel.bitmask &
...
bool LeAudioOffloadAudioProvider::isMatchedCodecFramesPerSDU(
        ...
        CodecSpecificCapabilitiesLtv::SupportedMaxCodecFramesPerSDU& capability_frame_sdu) {
...
bool LeAudioOffloadAudioProvider::isMatchedOctetsPerCodecFrame(
        ...
        CodecSpecificCapabilitiesLtv::SupportedOctetsPerCodecFrame& capability_octets) {
    return cfg_octets.value >= capability_octets.min && cfg_octets.value <= capability_octets.max;
}
...
bool LeAudioOffloadAudioProvider::isCapabilitiesMatchedCodecConfiguration(
    ...
    for (auto codec_cfg_data : codec_cfg) cfg_tag_map[codec_cfg_data.getTag()] = codec_cfg_data;
        ...
        switch (codec_capability.getTag()) {
            ...
            case CodecSpecificCapabilitiesLtv::Tag::supportedFrameDurations: {
                if (!isMatchedFrameDuration(
                            cfg->second.get<CodecSpecificConfigurationLtv::Tag::frameDuration>(),
                            codec_capability.get<CodecSpecificCapabilitiesLtv::Tag::
            ...
            case CodecSpecificCapabilitiesLtv::Tag::supportedOctetsPerCodecFrame: {
                if (!isMatchedOctetsPerCodecFrame(
                            cfg->second
                                    .get<CodecSpecificConfigurationLtv::Tag::octetsPerCodecFrame>(),
                            codec_capability.get<CodecSpecificCapabilitiesLtv::Tag::
...
bool LeAudioOffloadAudioProvider::filterMatchedAseConfiguration(
        LeAudioAseConfiguration& setting_cfg, const LeAudioAseConfiguration& requirement_cfg) {
    ...
    if (requirement_cfg.codecId.has_value()) {
        ...
        if (!isMatchedValidCodec(setting_cfg.codecId.value(), requirement_cfg.codecId.value())) {
            return false;
        }
    }
    ...
    std::map<CodecSpecificConfigurationLtv::Tag, CodecSpecificConfigurationLtv> cfg_tag_map;
    for (auto cfg : setting_cfg.codecConfiguration) cfg_tag_map[cfg.getTag()] = cfg;
...
bool LeAudioOffloadAudioProvider::isMatchedBISConfiguration(
    ...
    if (!isCapabilitiesMatchedCodecConfiguration(bis_cfg.codecConfiguration,
...
void LeAudioOffloadAudioProvider::filterCapabilitiesAseDirectionConfiguration(
        ...
        const IBluetoothAudioProvider::LeAudioDeviceCapabilities& capabilities,
        std::vector<std::optional<AseDirectionConfiguration>>& valid_direction_configurations) {
    for (auto direction_configuration : direction_configurations) {
        if (!direction_configuration.has_value()) continue;
        if (!direction_configuration.value().aseConfiguration.codecId.has_value()) continue;
        if (!isMatchedValidCodec(direction_configuration.value().aseConfiguration.codecId.value(),
                                 capabilities.codecId))
            continue;
        if (direction_configuration.value().aseConfiguration.codecId.value().getTag() ==
            CodecId::vendor) {
...
int getLeAudioAseConfigurationAllocationBitmask(LeAudioAseConfiguration cfg) {
    for (auto cfg_ltv : cfg.codecConfiguration) {
        if (cfg_ltv.getTag() == CodecSpecificConfigurationLtv::Tag::audioChannelAllocation) {
            return cfg_ltv.get<CodecSpecificConfigurationLtv::Tag::audioChannelAllocation>()
                    .bitmask;
        }
    }
    return 0;
...
std::optional<AseDirectionConfiguration> findValidMonoConfig(
        std::vector<AseDirectionConfiguration>& valid_direction_configurations, int bitmask) {
    for (auto& cfg : valid_direction_configurations) {
        int cfg_bitmask = getLeAudioAseConfigurationAllocationBitmask(cfg.aseConfiguration);
        if (getCountFromBitmask(cfg_bitmask) <= 1) {
            ...
            for (auto& codec_cfg : cfg.aseConfiguration.codecConfiguration) {
                if (codec_cfg.getTag() ==
                    CodecSpecificConfigurationLtv::Tag::audioChannelAllocation) {
                    codec_cfg.get<CodecSpecificConfigurationLtv::Tag::audioChannelAllocation>()
                            .bitmask = bitmask;
                    return cfg;
                }
            }
...
std::vector<AseDirectionConfiguration> getValidConfigurationsFromAllocation(
        ...
        std::vector<AseDirectionConfiguration>& valid_direction_configurations, bool isExact) {
    ...
    if (channel_count <= 1) {
        ...
        auto cfg = findValidMonoConfig(valid_direction_configurations, req_allocation_bitmask);
        ...
        for (int bit = 0; bit < 32; ++bit)
            if (req_allocation_bitmask & (1 << bit)) {
                auto cfg = findValidMonoConfig(valid_direction_configurations, (1 << bit));
...
void LeAudioOffloadAudioProvider::filterRequirementAseDirectionConfigurationExact(
    ...
    for (int i = 0; i < requirements.size(); ++i) {
        ...
        auto req_allocation_bitmask =
                getLeAudioAseConfigurationAllocationBitmask(requirement.value().aseConfiguration);
...
void LeAudioOffloadAudioProvider::filterRequirementAseDirectionConfigurationLoose(
        std::optional<std::vector<std::optional<AseDirectionConfiguration>>>&
                direction_configurations,
        const std::vector<std::optional<AseDirectionRequirement>>& requirements,
        std::optional<std::vector<std::optional<AseDirectionConfiguration>>>&
    ...
    for (auto& requirement : requirements) {
        ...
        auto req_valid_configs =
                getValidConfigurationsFromAllocation(req_allocation_bitmask, temp, false);
...
void LeAudioOffloadAudioProvider::filterRequirementAseDirectionConfiguration(
    ...
    if (!valid_direction_configurations.has_value()) {
        valid_direction_configurations = std::vector<std::optional<AseDirectionConfiguration>>();
    }

    if (isExact) {
        filterRequirementAseDirectionConfigurationExact(direction_configurations, requirements,
                                                        valid_direction_configurations);
    } else {
        filterRequirementAseDirectionConfigurationLoose(direction_configurations, requirements,
                                                        valid_direction_configurations);
    }
}
...
LeAudioOffloadAudioProvider::getCapabilitiesMatchedAseConfigurationSettings(
        ...
        const IBluetoothAudioProvider::LeAudioDeviceCapabilities& capabilities, uint8_t direction) {
    ...
    if (direction == kLeAudioDirectionSink) {
        ...
        if (!filtered_setting.sourceAseConfiguration.has_value()) return std::nullopt;
...
LeAudioOffloadAudioProvider::getRequirementMatchedAseConfigurationSettings(
        IBluetoothAudioProvider::LeAudioAseConfigurationSetting& setting,
        const IBluetoothAudioProvider::LeAudioConfigurationRequirement& requirement, bool isExact) {
    ...
    if (requirement.sourceAseRequirement.has_value()) {
        filterRequirementAseDirectionConfiguration(
                setting.sourceAseConfiguration, requirement.sourceAseRequirement.value(),
        ...
        if (!filtered_setting.sourceAseConfiguration.has_value()) {
            return std::nullopt;
               ...
               (requirement.flags.value().bitmask & ConfigurationFlags::SPATIAL_AUDIO) &&
        ...
        for (auto direction_configuration : setting.sourceAseConfiguration.value()) {
            if (direction_configuration.has_value()) {
                filtered_setting.sourceAseConfiguration.value().push_back(direction_configuration);
            }
        }
    }
...
std::optional<std::pair<std::string, IBluetoothAudioProvider::LeAudioAseConfigurationSetting>>
LeAudioOffloadAudioProvider::matchWithRequirement(
        std::vector<
                std::pair<std::string, IBluetoothAudioProvider::LeAudioAseConfigurationSetting>>&
    ...
    LOG(INFO) << __func__ << ": Trying to match for the requirement " << requirement.toString()
              << ", match context = " << isMatchContext << ", match exact = " << isExact
    ...
    for (auto& [setting_name, setting] : matched_ase_configuration_settings) {
        ...
        if (isMatchContext) {
            if ((setting.audioContext.bitmask & requirement.audioContext.bitmask) !=
                ...
                continue;
            LOG(DEBUG) << __func__ << ": Setting with matched context: name: " << setting_name
        ...
        if (isMatchFlags) {
            ...
            LOG(DEBUG) << __func__ << ": Setting with matched flags: name: " << setting_name
                       << ", setting: " << setting.toString();
        }
        ...
        if (filtered_ase_configuration_setting.has_value()) {
            LOG(INFO) << __func__ << ": Result found: name: " << setting_name
                      << ", setting: " << filtered_ase_configuration_setting.value().toString();
...
ndk::ScopedAStatus LeAudioOffloadAudioProvider::getLeAudioAseConfiguration(
    ...
    std::vector<std::pair<std::string, IBluetoothAudioProvider::LeAudioAseConfigurationSetting>>
            ase_configuration_settings = BluetoothAudioCodecs::GetLeAudioAseConfigurationSettings();
    ...
    if (in_remoteSinkAudioCapabilities.has_value()) {
        for (auto& [setting_name, setting] : ase_configuration_settings) {
            for (auto& capability : in_remoteSinkAudioCapabilities.value()) {
                ...
                auto filtered_ase_configuration_setting =
                        getCapabilitiesMatchedAseConfigurationSettings(setting, capability.value(),
            ...
            for (bool match_context : {true, false}) {
                for (bool match_flag : {true, false}) {
                    auto matched_setting =
                            matchWithRequirement(matched_ase_configuration_settings, requirement,
                                                 match_context, match_exact, match_flag);
    ...
    LOG(INFO) << __func__ << ": Found matches for all requirements, chosen settings:";
    for (auto& [setting_name, setting] : result) {
        LOG(INFO) << __func__ << ": name: " << setting_name << ", setting: " << setting.toString();
...
bool LeAudioOffloadAudioProvider::isMatchedQosRequirement(
        LeAudioAseQosConfiguration setting_qos, AseQosDirectionRequirement requirement_qos) {
    if (setting_qos.retransmissionNum != requirement_qos.preferredRetransmissionNum) {
        return false;
    }
...
bool isValidQosRequirement(AseQosDirectionRequirement qosRequirement) {
    return ((qosRequirement.maxTransportLatencyMs > 0) &&
            ...
            (qosRequirement.presentationDelayMaxUs >= qosRequirement.presentationDelayMinUs));
}

std::optional<LeAudioAseQosConfiguration> LeAudioOffloadAudioProvider::getDirectionQosConfiguration(
        uint8_t direction,
        const IBluetoothAudioProvider::LeAudioAseQosConfigurationRequirement& qosRequirement,
    ...
    std::optional<AseQosDirectionRequirement> direction_qos_requirement = std::nullopt;
    ...
    for (auto& [setting_name, setting] : ase_configuration_settings) {
        ...
        for (auto& cfg : direction_configuration.value()) {
            ...
            if (filterMatchedAseConfiguration(cfg.value().aseConfiguration,
        ...
        if (req_valid_configs.empty()) {
            LOG(WARNING) << __func__ << ": Cannot find matching allocation for bitmask "
...
ndk::ScopedAStatus LeAudioOffloadAudioProvider::getLeAudioAseQosConfiguration(
    ...
    if (in_qosRequirement.sourceAseQosRequirement.has_value()) {
        if (!isValidQosRequirement(in_qosRequirement.sourceAseQosRequirement.value()))
            return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
        bool found = false;
        for (bool match_flag : {true, false}) {
            for (bool match_exact : {true, false}) {
...
ndk::ScopedAStatus LeAudioOffloadAudioProvider::onSinkAseMetadataChanged(
        IBluetoothAudioProvider::AseState in_state, int32_t /*in_cigId*/, int32_t /*in_cisId*/,
...
LeAudioBroadcastConfigurationSetting getDefaultBroadcastSetting(
    ...
    if (context_bitmask & (AudioContext::LIVE_AUDIO | AudioContext::GAME)) {
        ...
        setting.maxTransportLatencyMs = 10;
        setting.maxSduOctets = 40;
    } else if (context_bitmask & (AudioContext::SOUND_EFFECTS | AudioContext::UNSPECIFIED)) {
        setting.retransmitionNum = 4;
        setting.maxTransportLatencyMs = 60;
        setting.maxSduOctets = 80;
    } else if (context_bitmask & (AudioContext::ALERTS | AudioContext::NOTIFICATIONS |
...
void modifySubBISConfigAllocation(
    ...
    for (auto& codec_cfg : sub_bis_cfg.bisConfiguration.codecConfiguration) {
        if (codec_cfg.getTag() == CodecSpecificConfigurationLtv::audioChannelAllocation) {
            codec_cfg.get<CodecSpecificConfigurationLtv::audioChannelAllocation>().bitmask =
                    allocation_bitmask;
            break;
        }
    }
...
void modifySubgroupConfiguration(
        IBluetoothAudioProvider::LeAudioBroadcastSubgroupConfiguration& subgroup_cfg,
        int context_bitmask) {
    // STEREO configs
    // Split into 2 sub BIS config, each has numBis = 1
    if (context_bitmask &
        (AudioContext::LIVE_AUDIO | AudioContext::GAME | AudioContext::SOUND_EFFECTS |
         AudioContext::UNSPECIFIED | AudioContext::MEDIA)) {
        if (subgroup_cfg.bisConfigurations.size() == 1)
            subgroup_cfg.bisConfigurations.push_back(subgroup_cfg.bisConfigurations[0]);
    ...
    for (auto& sub_bis_cfg : subgroup_cfg.bisConfigurations) {
        ...
        modifySubBISConfigAllocation(
                sub_bis_cfg, CodecSpecificConfigurationLtv::AudioChannelAllocation::FRONT_CENTER);
    }
}
...
void LeAudioOffloadAudioProvider::getBroadcastSettings() {
    ...
    LOG(INFO) << __func__ << ": Loading basic broadcast settings from provider info";
    ...
    for (auto& codec_info : db_codec_info) {
        if (codec_info.transport.getTag() != CodecInfo::Transport::leAudio) continue;
    ...
    LOG(INFO) << __func__ << ": Done loading broadcast settings from provider info";
}
...
LeAudioOffloadAudioProvider::getCapabilitiesMatchedBroadcastConfigurationSettings(
        LeAudioBroadcastConfigurationSetting& setting,
        const IBluetoothAudioProvider::LeAudioDeviceCapabilities& capabilities) {
    std::vector<IBluetoothAudioProvider::LeAudioBroadcastSubgroupConfiguration> filter_subgroup;
    for (auto& sub_cfg : setting.subgroupsConfigurations) {
        std::vector<IBluetoothAudioProvider::LeAudioSubgroupBisConfiguration> filtered_bis_cfg;
...
std::vector<CodecSpecificConfigurationLtv> getCodecRequirementBasedOnContext(
    ...
    if (quality == IBluetoothAudioProvider::BroadcastQuality::HIGH) {
        LOG(INFO) << __func__ << ": High quality, returning high quality requirement";
        requirement = {
                CodecSpecificConfigurationLtv::SamplingFrequency::HZ48000,
                CodecSpecificConfigurationLtv::FrameDuration::US10000,
        };
...
bool LeAudioOffloadAudioProvider::isSubgroupConfigurationMatchedContext(
        AudioContext requirement_context, IBluetoothAudioProvider::BroadcastQuality quality,
    ...
    std::map<CodecSpecificConfigurationLtv::Tag, CodecSpecificConfigurationLtv> req_tag_map;
        ...
        for (auto& x : bis_cfg.bisConfiguration.codecConfiguration) {
            ...
            if (p->second != x) {
                LOG(WARNING) << __func__ << ": does not match for context "
                             << requirement_context.toString() << ", cfg = " << x.toString();
                return false;
            }
        }
    }
...
ndk::ScopedAStatus LeAudioOffloadAudioProvider::getLeAudioBroadcastConfiguration(
        const std::optional<
                std::vector<std::optional<IBluetoothAudioProvider::LeAudioDeviceCapabilities>>>&
                in_remoteSinkAudioCapabilities,
    ...
    if (!in_remoteSinkAudioCapabilities.has_value() ||
        ...
        for (auto& setting : broadcast_settings) {
            for (auto& capability : in_remoteSinkAudioCapabilities.value()) {
                if (!capability.has_value()) continue;
                auto filtered_setting = getCapabilitiesMatchedBroadcastConfigurationSettings(
    ...
    auto context_bitmask = in_requirement.subgroupConfigurationRequirements[0].audioContext.bitmask;
    ...
    for (auto& sub_req : in_requirement.subgroupConfigurationRequirements) {
        ...
        for (auto setting : filtered_settings) {
            ...
            for (auto& sub_group_config : setting.subgroupsConfigurations) {
                if (!isSubgroupConfigurationMatchedContext(sub_req.audioContext, sub_req.quality,
                                                           sub_group_config)) {
                    is_matched = false;
                    break;
                }
        ...
        if (!is_setting_matched) {
            LOG(WARNING) << __func__ << ": Cannot find a setting that match requirement "
                         << sub_req.toString();
            return ndk::ScopedAStatus::ok();
        }
    }
    ...
    LOG(INFO) << __func__ << ": Combined setting that match: " << return_setting.toString();
